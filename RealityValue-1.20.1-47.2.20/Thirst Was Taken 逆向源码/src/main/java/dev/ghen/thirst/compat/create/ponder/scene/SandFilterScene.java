...
/* loaded from: ThirstWasTaken-1.20.1-1.4.0.jar:dev/ghen/thirst/compat/create/ponder/scene/SandFilterScene.class */
public class SandFilterScene {
    public static void filtering(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.overlay().showText(80).pointAt(filterSide).placeNearTarget().attachKeyFrame().text("A Sand Filter purifies by one step water pumped through it");
        scene.idle(70);
        ...
        scene.idle(5);
        FluidStack content = new FluidStack(Fluids.f_76193_, 4000);
        scene.world().modifyBlockEntity(topTankPos, FluidTankBlockEntity.class, be -> {
        ...
        scene.overlay().showText(60).pointAt(topTankSide).placeNearTarget().attachKeyFrame().text("Dirty Water needs to be pumped in from the top...");
        scene.idle(30);
        ...
        scene.overlay().showText(60).pointAt(bottomTankSide).placeNearTarget().attachKeyFrame().text("and Purified Water can be pumped out from the bottom");
        scene.idle(10);
        scene.world().modifyBlockEntity(topTankPos, FluidTankBlockEntity.class, be2 -> {
            be2.getTankInventory().drain(4000, IFluidHandler.FluidAction.EXECUTE);
        });
        scene.world().modifyBlockEntity(bottomTankPos, FluidTankBlockEntity.class, be3 -> {
            be3.getTankInventory().fill(content, IFluidHandler.FluidAction.EXECUTE);
        });
        scene.idle(50);
    }
}
