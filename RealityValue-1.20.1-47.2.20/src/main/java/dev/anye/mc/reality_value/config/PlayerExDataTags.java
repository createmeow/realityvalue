...
public class PlayerExDataTags extends _JsonConfig<HashMap<String, PlayerExDataTags.Range>> {
    public static final PlayerExDataTags HEALTH_TAGS = new PlayerExDataTags(_File.getFilePath(RealityValue.CONFIG_DATA_DIR,"health.json"), """
            {
            ...
            """);
    public static final PlayerExDataTags SANITY_TAGS = new PlayerExDataTags(_File.getFilePath(RealityValue.CONFIG_DATA_DIR,"sanity.json"), """
            {
    ...
    public Range getValue(Holder<Item> item){
        ...
        item.getTagKeys().forEach(itemTagKey -> {
    ...
    public record Range(int min, int max,CompoundTag nbt){
        ...
        public int getValue(){
            //return level.getRandom().nextInt(Math.abs(min),Math.abs(max + 1));
        ...
        public boolean checkNbt(CompoundTag itemNbt){
            ...
            for (String key : nbt.getAllKeys()){
                if (itemNbt.contains(key)){
                    ...
                    if (!tag.equals(nbt.get(key))) return false;
